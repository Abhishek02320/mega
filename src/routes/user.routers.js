...
router.route("/register").post(
    ...
    // yaha jo hamne middleware use kiya h jaise (upload)as middleware use kkya h jisme (milter) ka use kar rahe h as function for uploading file yaha (upload.) likhte hi bahut saare options milenge like (array, single, fields) jisme (array) mean aap ak hi field me ak se jyada file upload kar skte h (single)mean ak field me ak hi file upload ho bas (field)mean ak field me ak se jayada (arry)create karna jo apne ander apni hi tarah ki file ko accept kare doosare ki nahi // aap ak field me ak se jayada baar file upload kar sakte h but har baar puraani file store ya delete hone ke baad mean jab tak puraani file hamari localstore me store nahi hogi ya delete nahi hogi usse pahle koi nayi upload nahi ki jaayegi jabki agar ham (.array) ka use karte to ham ak baar me ak field me ak se jayada files ko store kara sakte the    
{
